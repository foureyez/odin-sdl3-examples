...
batched_quad :: proc() {
	...
	vertices := []PositionTextureVertex {
		{position = {-0.5, 0.5, 0}, uv = {0, 0}, tex_id = 1},
		{position = {0.5, 0.5, 0}, uv = {1, 0}, tex_id = 1},
		{position = {0.5, -0.5, 0}, uv = {1, 1}, tex_id = 1},
		{position = {-0.5, -0.5, 0}, uv = {0, 1}, tex_id = 1},
	}
	...
	sdl.UnmapGPUTransferBuffer(ctx.device, texture_transfer_buffer) // Unmap transfer bufffer
	...
	for is_running {
		...
		log.infof("Frame time: %s", time.since(start_time))
	}
}

destroy_batched_quad :: proc() {
	if ctx.window != nil {
